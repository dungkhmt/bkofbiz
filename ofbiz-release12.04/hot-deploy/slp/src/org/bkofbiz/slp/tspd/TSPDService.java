...
public class TSPDService {
	...
	///tsp-with-drone-tspdls-input-distance
	public static Map<String, Object> tspdSolveLS(DispatchContext dctx,
			Map<String, ? extends Object> context) {
		Delegator delegator = dctx.getDelegator();
		...
		dataJson2.addProperty("truckSpeed", truckSpeed);
		dataJson2.addProperty("droneSpeed", droneSpeed);
		dataJson2.addProperty("truckCost", truckCost);
		dataJson2.addProperty("delta", delta);
		dataJson2.addProperty("droneCost", droneCost);
		dataJson2.addProperty("endurance", endurance);
		dataJson2.addProperty("datasetid", datasetid);
		Map<String, Object> suc = ServiceUtil.returnSuccess();
		Gson gson = new Gson();
		List<Map<String, Object>> listDirection = null;
		List<Map<String, Object>> listPoint = null;
		 ...
		 */
		JsonParser jsonParser = new JsonParser();
		JsonObject jsonObject = dataJson2;
		jsonObject.remove("datasetid");
		jsonObject.add("listPoints", gson.toJsonTree(listPointMaped));
		jsonObject.add("map", gson.toJsonTree(map));
		/**
		...
		client.setWriteTimeout(100000, TimeUnit.SECONDS);
		RequestBody body = RequestBody.create(JSON_HEADER,
				jsonObject.toString());
		Debug.log(jsonObject.toString(), MODULE_NAME);
		Request request = new Request.Builder()
				.url("http://localhost:8088/ezRoutingAPI/tsp-with-drone-tspdls-input-distance")
				.post(body).build();
		Response response = null;
		String resultString = null;
		try {
