...
public class LoginServices {
	...
	public static Map<String, Object> directUpdatePassword(DispatchContext ctx, Map<String, ? extends Object> context){
		...
		try{
			...
			retSucc.put("result","Successfully");
		}catch(Exception ex){
			ex.printStackTrace();
	...
	public static String hashPassword(String password) {
		try {
			...
			String hasedPassword = HashCrypt.cryptPassword(getHashType(),
					password);
			hashedPassword = "{" + getHashType() + "}" + hashedPassword;
			Debug.log(module + "::hashPassword, hashType = " + getHashType()
					+ ", password = " + password + ", hasedPassword = "
					+ hasedPassword + ", hashedPassword = " + hashedPassword);
	...
	public static Map<String, Object> userLogin(DispatchContext ctx,
		...
		try {
			...
			String hasedPassword = HashCrypt.cryptPassword(getHashType(),
					password);
			Debug.log(module + "::userLogin, hashType = " + getHashType()
					+ ", password = " + password + ", hasedPassword = "
					+ hasedPassword + ", hashedPassword = " + hashedPassword);
		} catch (Exception ex) {
			ex.printStackTrace();
		...
		if (username == null || username.length() <= 0) {
			...
			if ("true".equalsIgnoreCase(UtilProperties.getPropertyValue(
					"security.properties", "username.lowercase"))) {
				username = username.toLowerCase();
			}
			if ("true".equalsIgnoreCase(UtilProperties.getPropertyValue(
					"security.properties", "password.lowercase"))) {
			...
			while (repeat) {
				...
				if (userLogin != null) {
					String ldmStr = UtilProperties.getPropertyValue(
							"security.properties", "login.disable.minutes");
					...
					Timestamp disabledDateTime = userLogin
							.getTimestamp("disabledDateTime");
					Timestamp reEnableTime = null;

					if (loginDisableMinutes > 0 && disabledDateTime != null) {
						reEnableTime = new Timestamp(disabledDateTime.getTime()
								+ loginDisableMinutes * 60000);
					}
					...
					boolean isSystem = (isServiceAuth && userLogin
							.get("isSystem") != null) ? "Y"
							.equalsIgnoreCase(userLogin.getString("isSystem"))
							: false;

					// grab the hasLoggedOut flag
					boolean hasLoggedOut = userLogin.get("hasLoggedOut") != null ? "Y"
							.equalsIgnoreCase(userLogin
									.getString("hasLoggedOut")) : false;

					if (UtilValidate.isEmpty(userLogin.getString("enabled"))
							|| "Y".equals(userLogin.getString("enabled"))
							|| (reEnableTime != null && reEnableTime
									.before(UtilDateTime.nowTimestamp()))
							|| (isSystem)) {
						...
						if ((!authFatalError && externalAuth)
							...
							Debug.logVerbose(
									"[LoginServices.userLogin] : Password Matched",
									module);
							...
							Long currentFailedLogins = userLogin
									.getLong("successiveFailedLogins");
							...
							if (!isServiceAuth) {
								...
								// service auth
								Map<?, ?> userLoginSessionMap = LoginWorker
										.getUserLoginSession(userLogin);

								// return the UserLoginSession Map
								if (userLoginSessionMap != null) {
									result.put("userLoginSession",
											userLoginSessionMap);
								}
							}
							...
							// encodedPassword +
							// "], Entered password OldFunnyHexEncode [" +
							...
							if (!authFatalError && isServiceAuth
									&& passNumber <= 1) {
								delegator
										.clearCacheLine("UserLogin", UtilMisc
												.toMap("userLoginId", username));
								repeat = true;
								continue;
							}
							...
							if (currentFailedLogins == null) {
								...
								currentFailedLogins = Long
										.valueOf(currentFailedLogins
												.longValue() + 1);
							}
							userLogin.set("successiveFailedLogins",
									currentFailedLogins);
							...
							// disable account
							String mflStr = UtilProperties.getPropertyValue(
									"security.properties", "max.failed.logins");
							...
							if (maxFailedLogins > 0
									&& currentFailedLogins.longValue() >= maxFailedLogins) {
						...
						try {
							...
							try {
								...
								if ("true".equals(UtilProperties
									...
									if (createHistory) {
										Map<String, Object> ulhCreateMap = UtilMisc
												.toMap("userLoginId",
														username,
														"visitId",
														visitId,
										...
										ModelEntity modelUserLogin = userLogin
												.getModelEntity();
										if (modelUserLogin.isField("partyId")) {
											ulhCreateMap.put("partyId",
								...
								try {
									...
									Debug.logError(e2,
											"Could not rollback nested transaction: "
								...
								try {
									...
									Debug.logError(e,
											"Could not commit nested transaction: "
													+ e.getMessage(), module);
								}
							}
						} finally {
							...
							if (parentTx != null) {
								try {
									...
									Debug.logVerbose(
											"Resumed the parent transaction.",
											module);
								} catch (GenericTransactionException e) {
									Debug.logError(e,
											"Could not resume parent nested transaction: "
													+ e.getMessage(), module);
								}
							}
						}
						...
						Map<String, Object> messageMap = UtilMisc
								.<String, Object> toMap("username", username);
						errMsg = UtilProperties
								.getMessage(
										resource,
										"loginservices.account_for_user_login_id_disabled",
										messageMap, locale);
						if (disabledDateTime != null) {
							messageMap = UtilMisc.<String, Object> toMap(
									"disabledDateTime", disabledDateTime);
							errMsg += " "
									+ UtilProperties.getMessage(resource,
											"loginservices.since_datetime",
											messageMap, locale);
						} else {
							errMsg += ".";
						}
						...
						if (loginDisableMinutes > 0 && reEnableTime != null) {
							...
							errMsg += " "
									+ UtilProperties.getMessage(resource,
											"loginservices.will_be_reenabled",
											messageMap, locale);
						} else {
							errMsg += " "
									+ UtilProperties
					...
					try {
						...
						errMsg = e.getMessage();
						Debug.logError(e,
								"External Authenticator had fatal exception : "
										+ e.getMessage(), module);
					}
					if (externalAuth) {
						...
						Debug.logInfo(
								"[LoginServices.userLogin] : Invalid User : "
										+ errMsg, module);
					}
				}
			}
	...
	private static void createUserLoginPasswordHistory(Delegator delegator,
		...
		EntityListIterator eli = delegator.find("UserLoginPasswordHistory",
				EntityCondition.makeConditionMap("userLoginId", userLoginId),
				null, null, UtilMisc.toList("-fromDate"), efo);
		...
		userLoginPwdHistToCreate.set(
				"currentPassword",
				useEncryption ? HashCrypt.cryptPassword(getHashType(),
	 ...
	 */
	public static Map<String, Object> createUserLogin(DispatchContext ctx,
			Map<String, ?> context) {
		Map<String, Object> result = FastMap.newInstance();
		Delegator delegator = ctx.getDelegator();
		Security security = ctx.getSecurity();
		GenericValue loggedInUserLogin = (GenericValue) context
				.get("userLogin");
		...
		String currentPasswordVerify = (String) context
				.get("currentPasswordVerify");
		String enabled = (String) context.get("enabled");
		String passwordHint = (String) context.get("passwordHint");
		String requirePasswordChange = (String) context
		...
		try {
			...
			Debug.logInfo(e,
					"Exception thrown while setting UserLogin partyId field: ",
					module);
		}
	...
	public static Map<String, Object> updatePassword(DispatchContext ctx,
		...
		Map<String, Object> result = ServiceUtil
				.returnSuccess(UtilProperties
						.getMessage(
								resource,
								"loginevents.password_was_changed_with_success",
								locale));
		...
		try {
			...
			Map<String, String> messageMap = UtilMisc.toMap("errorMessage",
					e.getMessage());
			errMsg = UtilProperties.getMessage(resource,
					"loginservices.could_not_change_password_read_failure",
					messageMap, locale);
			return ServiceUtil.returnError(errMsg);
		}
		...
		if ("true".equals(UtilProperties.getPropertyValue(
				"security.properties", "password.lowercase"))) {
		...
		if (newPassword != null) {
			checkNewPassword(userLoginToUpdate, currentPassword, newPassword,
					newPasswordVerify, passwordHint, errorMessageList,
					adminUser, locale);
		}
		...
		if (UtilValidate.isNotEmpty(externalAuthId)) {
			...
			try {
				...
				errMsg = UtilProperties
						.getMessage(
								resource,
								"loginservices.could_not_change_password_write_failure",
								messageMap, locale);
				return ServiceUtil.returnError(errMsg);
			}
		} else {
			userLoginToUpdate.set(
					"currentPassword",
					useEncryption ? HashCrypt.cryptPassword(getHashType(),
	 ...
	 * @return Map with the result of the service, the output parameters
	 */
	public static Map<String, Object> updateUserLoginId(DispatchContext ctx,
			Map<String, ?> context) {
		Map<String, Object> result = FastMap.newInstance();
		Delegator delegator = ctx.getDelegator();
		GenericValue loggedInUserLogin = (GenericValue) context
		...
		if ((userLoginId != null)
				&& ("true".equals(UtilProperties.getPropertyValue(
						"security.properties", "username.lowercase")))) {
		...
		// same party
		try {
			newUserLogin = delegator.findOne("UserLogin", false, "userLoginId",
					userLoginId);
		} catch (GenericEntityException e) {
			Debug.logWarning(e, "", module);
			Map<String, String> messageMap = UtilMisc.toMap("errorMessage",
		...
		// Deactivate 'old' UserLogin and do not set disabledDateTime here,
		// otherwise the 'old' UserLogin would be reenabled by next login
	 ...
	 * @return Map with the result of the service, the output parameters
	 */
	public static Map<String, Object> updateUserLoginSecurity(
		...
		if (context.containsKey("disabledDateTime")) {
			userLoginToUpdate.set("disabledDateTime",
					context.get("disabledDateTime"), true);
		}
	...
	public static void checkNewPassword(GenericValue userLogin,
		...
		if (!ignoreCurrentPassword) {
			...
			boolean passwordMatches = checkPassword(
					userLogin.getString("currentPassword"), useEncryption,
					currentPassword);
	...
	private static boolean checkPassword(String oldPassword,
			boolean useEncryption, String currentPassword) {
